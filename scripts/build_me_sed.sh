...
# sed is used instead of node -e to avoid MSYS2/Windows path conversion issues
# that arise when bash paths are passed into Node.js on Git for Windows.
APPCONFIG="$DEST_DIR/static/client/appConfig.json"
if [ -f "$APPCONFIG" ]; then
    # Escape forward slashes in the URL for use as a sed replacement string.
    MAPSERVICE_BASE_ESC=$(echo "$MAPSERVICE_BASE" | sed 's|/|\\/|g')
    sed -i "s|\"mapserviceBase\":.*|\"mapserviceBase\": \"${MAPSERVICE_BASE_ESC}\",|" "$APPCONFIG"
