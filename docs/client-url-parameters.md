...
- **Genvägar** tolkas av ett eget, enklare kodspår i `PresetLinks.jsx` som inte går via en sidladdning och som bara läser `x`/`y`/`z` (`z` valfri, faller då tillbaka på kartans nuvarande zoom) — kräver `x`+`y` för att räknas som giltig. `l`/`gl` läses aldrig: en genväg flyttar bara kartan och rör aldrig tända lager eller bakgrundskarta, se [admin-tool-preset.md](admin-tool-preset.md).
- **Hemknappen** (`MapResetter`, se rad "Visa en hemknapp..." i [Kartinställningar → Inställningar](admin-mapoptions.md)) styrs _inte_ av dessa parametrar. Den återställer alltid till administratörens konfigurerade standardvärden för zoom/centrum, oavsett hur kartan öppnades — även om sidan laddades via en länk med `x`/`y`/`z`.
