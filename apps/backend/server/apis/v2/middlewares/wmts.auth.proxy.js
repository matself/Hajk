...
// Grab a logger
const logger = log4js.getLogger("proxy.wmtsAuth.v2");

// The public path under which this proxy is mounted (per API version this
// becomes e.g. /api/v2/wmtsproxy). The first path segment after the mount
// point is the Hajk layer id.
export const WMTS_AUTH_PROXY_PATH = "wmtsproxy";

// In-memory index: layerId -> { origin, authHeader }. Reading (and parsing)
// layers.json on every single tile request would be far too expensive, so we
...
function getIndex() {
  ...
  try {
    ...
    logger.error("Could not stat layers.json for WMTS auth proxy:", error);
  ...
  try {
    ...
    for (const layer of json.wmtslayers || []) {
      ...
      try {
        ...
        logger.warn(
          "Skipping WMTS layer %o in auth proxy: invalid url %o",
          layer.id,
          layer.url
        );
        continue;
    ...
    logger.debug(
      "Rebuilt WMTS auth proxy index with %d authenticated layer(s).",
      map.size
    );
  } catch (error) {
    logger.error("Failed to build WMTS auth proxy index:", error);
 ...
 * (…/wmtsproxy/<layerId>/<upstreamPath…>) rather than `req.url`, because by the
...
function layerIdFromReq(req) {
  ...
  const match = url.match(/\/wmtsproxy\/([^/?]+)/);
  return match ? decodeURIComponent(match[1]) : null;
}

/**
 * @summary Proxy WMTS tile requests to an upstream service that requires
 ...
 * browser). The client requests tiles from /api/v{version}/wmtsproxy/<layerId>/…
 * same-origin, so tiles load as ordinary <img> tags. This middleware looks up
 ...
 */
export default function wmtsAuthProxy() {
  return createProxyMiddleware({
    ...
    // upstream path (including the WMTS template placeholders / KVP query),
    ...
    on: {
      ...
      proxyRes: (proxyRes) => {
        // Allow the tiles to be used cross-origin (e.g. client dev server on a
      ...
      error: (err, req, res) => {
        ...
        if (res && !res.headersSent && typeof res.status === "function") {
          res.status(502).send("WMTS auth proxy: upstream request failed.");
        }
      },
    },
  });
